...
{ pub mod access
{ use futures::{{StreamExt, TryStreamExt}};use futures; use cornucopia_async::GenericClient;#[derive( Debug)] pub struct CreateParams<T1: cornucopia_async::StringSql,> { pub repository_uuid: uuid::Uuid,pub user_uuid: uuid::Uuid,pub access_level: T1,}#[derive(Clone,Copy, Debug)] pub struct DeleteByUserUuidAndRepositoryUuidParams<> { pub user_uuid: uuid::Uuid,pub repository_uuid: uuid::Uuid,}#[derive(Clone,Copy, Debug)] pub struct UserHasAccessParams<> { pub user_uuid: uuid::Uuid,pub repository_uuid: uuid::Uuid,}#[derive( Debug, Clone, PartialEq,)] pub struct DeleteByUserUuidAndRepositoryUuid
{ pub uuid : uuid::Uuid,pub repository_uuid : uuid::Uuid,pub user_uuid : uuid::Uuid,pub access_level : String,pub created_at : time::PrimitiveDateTime,pub updated_at : time::PrimitiveDateTime,}pub struct DeleteByUserUuidAndRepositoryUuidBorrowed<'a> { pub uuid : uuid::Uuid,pub repository_uuid : uuid::Uuid,pub user_uuid : uuid::Uuid,pub access_level : &'a str,pub created_at : time::PrimitiveDateTime,pub updated_at : time::PrimitiveDateTime,}
impl<'a> From<DeleteByUserUuidAndRepositoryUuidBorrowed<'a>> for DeleteByUserUuidAndRepositoryUuid
{
    fn from(DeleteByUserUuidAndRepositoryUuidBorrowed { uuid,repository_uuid,user_uuid,access_level,created_at,updated_at,}: DeleteByUserUuidAndRepositoryUuidBorrowed<'a>) -> Self
    { Self { uuid,repository_uuid,user_uuid,access_level: access_level.into(),created_at,updated_at,} }
}pub struct DeleteByUserUuidAndRepositoryUuidQuery<'a, C: GenericClient, T, const N: usize>
{
    client: &'a  C, params:
    [&'a (dyn postgres_types::ToSql + Sync); N], stmt: &'a mut
    cornucopia_async::private::Stmt, extractor: fn(&tokio_postgres::Row) -> DeleteByUserUuidAndRepositoryUuidBorrowed,
...
GenericClient
{
    pub fn map<R>(self, mapper: fn(DeleteByUserUuidAndRepositoryUuidBorrowed) -> R) ->
...
}#[derive( Debug, Clone, PartialEq,)] pub struct GetByUser
{ pub uuid : uuid::Uuid,pub repository_uuid : uuid::Uuid,pub user_uuid : uuid::Uuid,pub access_level : String,pub created_at : time::PrimitiveDateTime,pub updated_at : time::PrimitiveDateTime,}pub struct GetByUserBorrowed<'a> { pub uuid : uuid::Uuid,pub repository_uuid : uuid::Uuid,pub user_uuid : uuid::Uuid,pub access_level : &'a str,pub created_at : time::PrimitiveDateTime,pub updated_at : time::PrimitiveDateTime,}
impl<'a> From<GetByUserBorrowed<'a>> for GetByUser
{
    fn from(GetByUserBorrowed { uuid,repository_uuid,user_uuid,access_level,created_at,updated_at,}: GetByUserBorrowed<'a>) -> Self
    { Self { uuid,repository_uuid,user_uuid,access_level: access_level.into(),created_at,updated_at,} }
}pub struct GetByUserQuery<'a, C: GenericClient, T, const N: usize>
{
    client: &'a  C, params:
    [&'a (dyn postgres_types::ToSql + Sync); N], stmt: &'a mut
    cornucopia_async::private::Stmt, extractor: fn(&tokio_postgres::Row) -> GetByUserBorrowed,
    mapper: fn(GetByUserBorrowed) -> T,
} impl<'a, C, T:'a, const N: usize> GetByUserQuery<'a, C, T, N> where C:
GenericClient
{
    pub fn map<R>(self, mapper: fn(GetByUserBorrowed) -> R) ->
...
{ pub user_uuid : uuid::Uuid,pub access_level : String,pub username : String,}pub struct GetAllUsersWithAccessBorrowed<'a> { pub user_uuid : uuid::Uuid,pub access_level : &'a str,pub username : &'a str,}
impl<'a> From<GetAllUsersWithAccessBorrowed<'a>> for GetAllUsersWithAccess
{
    ...
    { Self { user_uuid,access_level: access_level.into(),username: username.into(),} }
...
{ CreateStmt(cornucopia_async::private::Stmt::new("INSERT INTO Access (repository_uuid, user_uuid, access_level)
    VALUES ($1, $2, $3)")) } pub struct
CreateStmt(cornucopia_async::private::Stmt); impl CreateStmt
{ pub async fn bind<'a, C:
GenericClient,T1:
cornucopia_async::StringSql,>(&'a mut self, client: &'a  C,
repository_uuid: &'a uuid::Uuid,user_uuid: &'a uuid::Uuid,access_level: &'a T1,) -> Result<u64, tokio_postgres::Error>
{
...
2>
{
    DeleteByUserUuidAndRepositoryUuidQuery
    {
        ...
        |row| { DeleteByUserUuidAndRepositoryUuidBorrowed { uuid: row.get(0),repository_uuid: row.get(1),user_uuid: row.get(2),access_level: row.get(3),created_at: row.get(4),updated_at: row.get(5),} }, mapper: |it| { <DeleteByUserUuidAndRepositoryUuid>::from(it) },
    }
...
{ pub fn bind<'a, C:
GenericClient,>(&'a mut self, client: &'a  C,
user_uuid: &'a uuid::Uuid,repository_uuid: &'a uuid::Uuid,) -> StringQuery<'a,C, String,
2>
{
    StringQuery
    {
...
UserHasAccessParams<>, StringQuery<'a, C, String,
2>, C> for UserHasAccessStmt
{
    fn
    params(&'a mut self, client: &'a  C, params: &'a
    UserHasAccessParams<>) -> StringQuery<'a, C,
    String, 2>
    { self.bind(client, &params.user_uuid,&params.repository_uuid,) }
...
1>
{
    GetByUserQuery
    {
        ...
        |row| { GetByUserBorrowed { uuid: row.get(0),repository_uuid: row.get(1),user_uuid: row.get(2),access_level: row.get(3),created_at: row.get(4),updated_at: row.get(5),} }, mapper: |it| { <GetByUser>::from(it) },
    }
...
{ use futures::{{StreamExt, TryStreamExt}};use futures; use cornucopia_async::GenericClient;#[derive( Debug, Clone, PartialEq,)] pub struct DeleteByUuid
...
GenericClient,T1:
cornucopia_async::StringSql,>(&'a mut self, client: &'a  C,
username: &'a T1,) -> Result<u64, tokio_postgres::Error>
{
    let stmt = self.0.prepare(client).await?;
