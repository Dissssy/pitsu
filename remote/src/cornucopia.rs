...
{
    fn from(GetBorrowed { uuid,repository_uuid,file_path,aws_s3_object_key,created_at,updated_at,}: GetBorrowed<'a>) -> Self
    { Self { uuid,repository_uuid,file_path: file_path.into(),aws_s3_object_key: aws_s3_object_key.into(),created_at,updated_at,} }
}pub struct GetQuery<'a, C: GenericClient, T, const N: usize>
{
    client: &'a  C, params:
    [&'a (dyn postgres_types::ToSql + Sync); N], stmt: &'a mut
    cornucopia_async::private::Stmt, extractor: fn(&tokio_postgres::Row) -> GetBorrowed,
    mapper: fn(GetBorrowed) -> T,
} impl<'a, C, T:'a, const N: usize> GetQuery<'a, C, T, N> where C:
GenericClient
{
    pub fn map<R>(self, mapper: fn(GetBorrowed) -> R) ->
    GetQuery<'a,C,R,N>
    {
        GetQuery
        {
...
}#[derive( Debug, Clone, PartialEq,)] pub struct GetAllIn
{ pub uuid : uuid::Uuid,pub repository_uuid : uuid::Uuid,pub file_path : String,pub aws_s3_object_key : String,pub created_at : time::PrimitiveDateTime,pub updated_at : time::PrimitiveDateTime,}pub struct GetAllInBorrowed<'a> { pub uuid : uuid::Uuid,pub repository_uuid : uuid::Uuid,pub file_path : &'a str,pub aws_s3_object_key : &'a str,pub created_at : time::PrimitiveDateTime,pub updated_at : time::PrimitiveDateTime,}
impl<'a> From<GetAllInBorrowed<'a>> for GetAllIn
{
    fn from(GetAllInBorrowed { uuid,repository_uuid,file_path,aws_s3_object_key,created_at,updated_at,}: GetAllInBorrowed<'a>) -> Self
    { Self { uuid,repository_uuid,file_path: file_path.into(),aws_s3_object_key: aws_s3_object_key.into(),created_at,updated_at,} }
}pub struct GetAllInQuery<'a, C: GenericClient, T, const N: usize>
{
    client: &'a  C, params:
    [&'a (dyn postgres_types::ToSql + Sync); N], stmt: &'a mut
    cornucopia_async::private::Stmt, extractor: fn(&tokio_postgres::Row) -> GetAllInBorrowed,
    mapper: fn(GetAllInBorrowed) -> T,
} impl<'a, C, T:'a, const N: usize> GetAllInQuery<'a, C, T, N> where C:
GenericClient
{
    pub fn map<R>(self, mapper: fn(GetAllInBorrowed) -> R) ->
    GetAllInQuery<'a,C,R,N>
    {
        GetAllInQuery
        {
            client: self.client, params: self.params, stmt: self.stmt,
...
{ pub uuid : uuid::Uuid,pub repository_uuid : uuid::Uuid,pub file_path : String,pub aws_s3_object_key : String,pub created_at : time::PrimitiveDateTime,pub updated_at : time::PrimitiveDateTime,}pub struct GetAllBorrowed<'a> { pub uuid : uuid::Uuid,pub repository_uuid : uuid::Uuid,pub file_path : &'a str,pub aws_s3_object_key : &'a str,pub created_at : time::PrimitiveDateTime,pub updated_at : time::PrimitiveDateTime,}
impl<'a> From<GetAllBorrowed<'a>> for GetAll
{
    fn from(GetAllBorrowed { uuid,repository_uuid,file_path,aws_s3_object_key,created_at,updated_at,}: GetAllBorrowed<'a>) -> Self
    { Self { uuid,repository_uuid,file_path: file_path.into(),aws_s3_object_key: aws_s3_object_key.into(),created_at,updated_at,} }
...
{
    fn from(DeleteBorrowed { uuid,repository_uuid,file_path,aws_s3_object_key,created_at,updated_at,}: DeleteBorrowed<'a>) -> Self
    { Self { uuid,repository_uuid,file_path: file_path.into(),aws_s3_object_key: aws_s3_object_key.into(),created_at,updated_at,} }
}pub struct DeleteQuery<'a, C: GenericClient, T, const N: usize>
{
    client: &'a  C, params:
    [&'a (dyn postgres_types::ToSql + Sync); N], stmt: &'a mut
    cornucopia_async::private::Stmt, extractor: fn(&tokio_postgres::Row) -> DeleteBorrowed,
    mapper: fn(DeleteBorrowed) -> T,
} impl<'a, C, T:'a, const N: usize> DeleteQuery<'a, C, T, N> where C:
GenericClient
{
    pub fn map<R>(self, mapper: fn(DeleteBorrowed) -> R) ->
    DeleteQuery<'a,C,R,N>
    {
        DeleteQuery
        {
...
GenericClient,T1:
cornucopia_async::StringSql,>(&'a mut self, client: &'a  C,
file_path: &'a T1,repository_uuid: &'a uuid::Uuid,) -> GetQuery<'a,C, Get,
2>
{
    GetQuery
    {
        client, params: [file_path,repository_uuid,], stmt: &mut self.0, extractor:
        |row| { GetBorrowed { uuid: row.get(0),repository_uuid: row.get(1),file_path: row.get(2),aws_s3_object_key: row.get(3),created_at: row.get(4),updated_at: row.get(5),} }, mapper: |it| { <Get>::from(it) },
    }
} }impl <'a, C: GenericClient,T1: cornucopia_async::StringSql,> cornucopia_async::Params<'a,
GetParams<T1,>, GetQuery<'a, C, Get,
2>, C> for GetStmt
{
    fn
    params(&'a mut self, client: &'a  C, params: &'a
    GetParams<T1,>) -> GetQuery<'a, C,
    Get, 2>
    { self.bind(client, &params.file_path,&params.repository_uuid,) }
}pub fn get_all_in() -> GetAllInStmt
{ GetAllInStmt(cornucopia_async::private::Stmt::new("SELECT * FROM Files WHERE repository_uuid = $1")) } pub struct
GetAllInStmt(cornucopia_async::private::Stmt); impl GetAllInStmt
{ pub fn bind<'a, C:
GenericClient,>(&'a mut self, client: &'a  C,
repository_uuid: &'a uuid::Uuid,) -> GetAllInQuery<'a,C, GetAllIn,
1>
{
    GetAllInQuery
    {
        client, params: [repository_uuid,], stmt: &mut self.0, extractor:
        |row| { GetAllInBorrowed { uuid: row.get(0),repository_uuid: row.get(1),file_path: row.get(2),aws_s3_object_key: row.get(3),created_at: row.get(4),updated_at: row.get(5),} }, mapper: |it| { <GetAllIn>::from(it) },
    }
} }pub fn get_all() -> GetAllStmt
{ GetAllStmt(cornucopia_async::private::Stmt::new("SELECT * FROM Files")) } pub struct
...
GenericClient,T1:
cornucopia_async::StringSql,>(&'a mut self, client: &'a  C,
file_path: &'a T1,repository_uuid: &'a uuid::Uuid,) -> DeleteQuery<'a,C, Delete,
2>
{
    DeleteQuery
    {
        client, params: [file_path,repository_uuid,], stmt: &mut self.0, extractor:
        |row| { DeleteBorrowed { uuid: row.get(0),repository_uuid: row.get(1),file_path: row.get(2),aws_s3_object_key: row.get(3),created_at: row.get(4),updated_at: row.get(5),} }, mapper: |it| { <Delete>::from(it) },
    }
} }impl <'a, C: GenericClient,T1: cornucopia_async::StringSql,> cornucopia_async::Params<'a,
DeleteParams<T1,>, DeleteQuery<'a, C, Delete,
2>, C> for DeleteStmt
{
    fn
    params(&'a mut self, client: &'a  C, params: &'a
    DeleteParams<T1,>) -> DeleteQuery<'a, C,
    Delete, 2>
    { self.bind(client, &params.file_path,&params.repository_uuid,) }
}}pub mod repository
